...
void drawbar (Monitor* m) {
    ...
    drw_setscheme (drw, scheme[SchemeNorm]);
    /* w = TEXTW (m->ltsymbol); */
    /* x = drw_text (drw, x, 2, w, bh, 0, m->ltsymbol, 0); */
    /* x = drw_text (drw, x, -1, w, bh, 0, m->ltsymbol, 0); */
...
void incnmaster (const Arg* arg) {
    selmon->nmaster = MIN (MAX (selmon->nmaster + arg->i, 1), 2);
    arrange (selmon);
}
...
void updatebars (void) {
    ...
    for (m = mons; m; m = m->next) {
        ...
        if (showsystray && m == systraytomon (m))
            w -= getsystraywidth ();
        m->barwin = XCreateWindow (dpy, root, m->wx, m->by, w, bh, 0, DefaultDepth (dpy, screen), CopyFromParent,
